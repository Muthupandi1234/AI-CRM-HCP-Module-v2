...
def local_ai_extractor(text: str):
    # Default values - None nu vechurukom (field kandupidikkalana null thaan pogum)
    hcp = None
    spec = None
    fac = None
    top = None
    ...
    if "clinic" in text_lower or "hospital" in text_lower:
        fac_match = re.search(r'([a-zA-Z\s]+(?:clinic|hospital))', text, re.IGNORECASE)
        if fac_match:
...
async def chat_endpoint(req: ChatRequest):
    ...
    # Enda field-um kandupidikkalana (None) na, adha response-la anuppave koodathu.
    # Ippadi pannaa dhaan frontend-la already fill aana values delete aagathu.
    raw_form_state = {
        "hcpName": hcp,
        "specialty": spec,
        "facility": fac,
    ...
    form_state = {k: v for k, v in raw_form_state.items() if v is not None}
 
    # Reply message-ku, edhavadhu name kidaikkalana generic-a pesuvom (None nu kaataamale)
    subject = hcp if hcp else "your entry"
    chat_reply = f"I have processed your sync notes for {subject}. The extraction workflow successfully completed and the database CRM record has been updated."
 
    return {
        "response": chat_reply,
        "form_state": form_state,   # only the fields we actually extracted this turn
        "analytics": {
